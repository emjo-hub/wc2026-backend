...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    for (let i=0;i<n;i++) {
      ...
      const ca = pSample(Math.max(0.5,(possA/100)*5.0*pfA)) + pSample(Math.max(0.5,(possA/100)*4.5*pfA));
const cb = pSample(Math.max(0.5,(possB/100)*4.5*pfB)) + pSample(Math.max(0.5,(possB/100)*5.0*pfB));
      const total = ca + cb;
